...
public class TriviaQuestionArrayAccess implements TriviaQuestionAccessible {
    ...
    public List<TriviaQuestion> getSpecifiedQuestionList(long... id) {
        ...
        //List<Object> ids = Arrays.stream(id).boxed().collect(Collectors.toList());
    	List<TriviaQuestion> ids = new ArrayList<>();
    	for(long currentId:id){
    		ids.add(getQuestionById(currentId)); 
    	}
